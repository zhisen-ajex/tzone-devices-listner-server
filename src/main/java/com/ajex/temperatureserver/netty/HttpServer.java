        ...
        if (webSocketConfiguration.isEnable()) {
            ...
            try {
                ...
                future.addListener(f -> {
                    if (f.isSuccess()) {
                        log.info("HttpServer - Start completed.");
                    } else {
                        throw new WebSocketException("HttpServer - Start failed.");
                    }
                });
                log.info("HttpServer starts to listen on the port: {}", webSocketConfiguration.getPort());
